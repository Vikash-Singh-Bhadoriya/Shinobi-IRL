    ...
    <section className="rasengan-debug" aria-label="Rasengan analysis">
      ...
      {result.map(({ hand, color, detection }) => (
        <div key={hand}>
          ...
          <div className="gesture-debug-row"><span className="gesture-debug-label">Pinch</span><span className="gesture-debug-value">{detection.pinchDetected ? 'YES' : 'NO'} ({detection.pinchRawDistance.toFixed(2)} raw / {detection.pinchDistance.toFixed(2)} smooth)</span></div>
          <div className="gesture-debug-row"><span className="gesture-debug-label">Pinch Thresholds</span><span className="gesture-debug-value">{detection.pinchStartThreshold.toFixed(2)} / {detection.pinchReleaseThreshold.toFixed(2)}</span></div>
          <div className="gesture-debug-row"><span className="gesture-debug-label">Pinch Duration</span><span className="gesture-debug-value">{Math.round(detection.pinchDurationMs)} ms</span></div>
